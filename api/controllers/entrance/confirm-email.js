...
(for existing users who just changed their email address).`,
  ...
  fn: async function (inputs, exits) {
    ...
    var user = await AclUsers.findOne({ emailProofToken: inputs.token });
    ...
    if (user.emailStatus === 'unconfirmed') {
      ...
      await AclUsers.update({ id: user.id }).set({
      ...
      if (await AclUsers.count({ emailAddress: user.emailChangeCandidate }) > 0) {
